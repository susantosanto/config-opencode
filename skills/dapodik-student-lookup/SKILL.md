---
name: dapodik-student-lookup
description: "Cari data lengkap siswa berdasarkan nama dan kelas dari Dapodik Web Service untuk keperluan surat-menyurat dan dokumen sekolah"
license: MIT
...
# Dapodik Student Lookup Skill

Cari data lengkap peserta didik berdasarkan nama dan/atau kelas dari Dapodik Web Service. Data yang diambil bisa digunakan untuk:
...
## Trigger Phrases
- "cari data siswa [nama] kelas [kelas]"
- "lookup siswa [nama]"
- "data lengkap siswa [nama]"
- "ambil data peserta didik [nama]"
- "cari peserta didik [nama] kelas [kelas]"
- "data siswa untuk surat"
...
Script tersedia di: `C:\Users\USER\.config\opencode\skills\dapodik-student-lookup\lookup.py`

```bash
python C:\Users\USER\.config\opencode\skills\dapodik-student-lookup\lookup.py "[nama_siswa]" "[kelas_opsional]"
```

### 3. Output
Script akan menampilkan data lengkap siswa dalam format yang siap digunakan untuk dokumen.
...
def format_data_siswa(siswa):
    ...
    output.append(f"  Jenis Kelamin   : {'Laki-laki' if siswa.get('jenis_kelamin') == 'L' else 'Perempuan' if siswa.get('jenis_kelamin') == 'P' else '-'}")
    ...
    output.append(f"  Lintang/Bujur   : {siswa.get('lintang', '-')}/{siswa.get('bujur', '-')}")
    output.append("")
...
def format_json_siswa(siswa):
    """Format data siswa sebagai JSON untuk integrasi"""
    return json.dumps(siswa, ensure_ascii=False, indent=2)
...
if __name__ == "__main__":
    ...
    for i, siswa in enumerate(hasil):
        ...
        if i == 0:
            with open(r"C:\Users\USER\Documents\dapodik_student_lookup_result.json", "w", encoding="utf-8") as f:
                json.dump({
                    "search_name": nama,
                    "search_class": kelas,
                    "found": len(hasil),
            ...
            print(f"Data JSON disimpan ke: C:\\Users\\USER\\Documents\\dapodik_student_lookup_result.json")
```

## API Endpoint Reference
- **URL**: `http://localhost:5774/WebService/getPesertaDidik`
...
1. Data Pribadi (nama, NISN, NIK, TTL, dll)
2. Data Akademik (kelas, kurikulum, tanggal masuk)
3. Data Orang Tua (ayah, ibu, wali + pekerjaan)
4. Data Alamat (alamat lengkap, koordinat)
5. Data Kontak (telepon, email)
6. Data ID (untuk referensi sistem)

## Integration
Hasil pencarian juga disimpan sebagai JSON di:
`C:\Users\USER\Documents\dapodik_student_lookup_result.json`
